	...
	m_pAnimStateData(nullptr)
{
	...
	for(uint32 i = 0; i < uiNumAtlases; ++i)
	{
		...
		if(sPath == HY_GUI_DATAOVERRIDE)
			...
			pAtlas = assetsRef.GetAtlas(atlasObj["checksum"].GetUint(), rSubAtlasUVRect);
#else
		pAtlas = assetsRef.GetAtlas(atlasObj["checksum"].GetUint(), rSubAtlasUVRect);
#endif

		// NOTE: pAtlas may be nullptr if this is a preview generated by the Editor Tool
		//
		...
		if(pAtlas)
		{
			...
			uiFullAtlasWidth = pAtlas->GetWidth();
			uiFullAtlasHeight = pAtlas->GetHeight();
		}
		else
		{
