...
HyScrollBar::HyScrollBar(HyOrientation eOrientation, const HyUiPanelInit &posBtnInit, const HyUiPanelInit &negBtnInit, const HyUiPanelInit &barInit, const HyUiPanelInit &handleInit, HyEntity2d *pParent) :
	...
	m_pCallbackData(nullptr)
{
	Setup(eOrientation, posBtnInit, negBtnInit, barInit, handleInit);
}
...
void HyScrollBar::Setup(HyOrientation eOrientation, const HyUiPanelInit &posBtnInit, const HyUiPanelInit &negBtnInit, const HyUiPanelInit &barInit, const HyUiPanelInit &handleInit)
{
	...
	{
		m_uiDiameter = posBtnInit.m_uiWidth;
	}
	else
	{
		m_uiDiameter = posBtnInit.m_uiHeight;
	}
	m_bIsDragging = false;
...
//{
//	if(m_BarPanel.IsPrimForPanel())
//		m_BarPanel.GetPanelNode()->SetLayerColor(0, color);
//
//	if(color.IsLight())
...
void HyScrollBar::OffsetSlider(float fPixels, bool bOffsetClientContentPixels)
{
	...
	if(bOffsetClientContentPixels == false)
	{
		...
		if(m_eOrientation == HYORIENT_Vertical)
			fScrollBarLength = m_BarPanel.GetHeight();
		else
			fScrollBarLength = m_BarPanel.GetWidth();
...
/*virtual*/ void HyScrollBar::OnAssemble() /*override*/
{
	if(GetOrientation() == HYORIENT_Vertical)
	{
		if(m_PosBtn.panel.IsPrimForPanel() && m_PosBtn.panel.GetPanelNode())
		{
...
/*virtual*/ void HyScrollBar::OnMouseDown() /*override*/
{
	HyScrollBar *pScrollBar = static_cast<HyScrollBar *>(m_pParent);
	...
	glm::vec2 vSliderSize = m_HandlePanel.GetSize();
	glm::vec2 ptSliderPos = m_HandlePanel.GetPosition();
	b2AABB sliderAABB = { { ptSliderPos.x, ptSliderPos.y }, { ptSliderPos.x + vSliderSize.x, ptSliderPos.y + vSliderSize.y } };
	if(pScrollBar->GetOrientation() == HYORIENT_Vertical)
	{
...
void HyScrollBar::InvokeOnScrollCallback()
{
	...
	//m_PageControl.SetSliderPos(m_eORIENTATION, , m_fClientTotalSize, m_fClientShownSize);
	
	if(GetOrientation() == HYORIENT_Vertical)
		m_HandlePanel.SetPosition(static_cast<float>(m_uiDiameter) * 0.25f, ((static_cast<float>(m_fClientTotalSize - m_fClientShownSize) - m_AnimScrollPos.Get()) * (m_BarPanel.GetHeight() - m_HandlePanel.GetHeight())) / static_cast<float>(m_fClientTotalSize - m_fClientShownSize));
	else
		m_HandlePanel.SetPosition((m_AnimScrollPos.Get() * (m_BarPanel.GetWidth() - m_HandlePanel.GetWidth())) / static_cast<float>(m_fClientTotalSize - m_fClientShownSize), static_cast<float>(m_uiDiameter) * 0.25f);
