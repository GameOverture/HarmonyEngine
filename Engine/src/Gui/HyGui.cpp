...
HyGui *HyGui::sm_pCurModalContainer = nullptr;
std::vector<HyGui *> HyGui::sm_pContainerList;
HySpacerHandle HyGui::sm_hSpacerHandleCounter = 1;
HyLayoutHandle HyGui::sm_hLayoutHandleCounter = 1;

HyGui::HyGui(HyOrientation eRootLayoutDirection, const HyUiPanelInit &initRef, HyEntity2d *pParent /*= nullptr*/) :
	IHyGuiBase(pParent),
...
/*static*/ bool HyGui::IsModalActive()
{
	return sm_pCurModalContainer != nullptr;
}

bool HyGui::Show(bool bInstant /*= false*/)
{
...
bool HyGui::Hide(bool bInstant /*= false*/)
{
...
/*static*/ void HyGui::RelinquishModal()
{
	for(uint32 i = 0; i < static_cast<uint32>(sm_pContainerList.size()); ++i)
	{
		IHyWidget *pFocusedWidget = sm_pContainerList[i]->GetFocusedWidget();
...
IHyWidget *HyGui::GetFocusedWidget()
{
...
IHyWidget *HyGui::FocusNextWidget(bool bForwardDirection)
{
	auto widgetList = AssembleWidgetList();
...
glm::vec2 HyGui::GetWidgetPos(IHyWidget &widgetRef)
{
...
bool HyGui::InsertWidget(IHyWidget &widgetRef, HyLayoutHandle hInsertInto /*= HY_UNUSED_HANDLE*/)
{
	bool bSuccess = false;
...
bool HyGui::RemoveWidget(IHyWidget &widgetRef)
{
...
HySpacerHandle HyGui::InsertSpacer(HySizePolicy eSizePolicy /*= HYSIZEPOLICY_Expanding*/, uint32 uiSizeHint /*= 0*/, HyLayoutHandle hInsertInto /*= HY_UNUSED_HANDLE*/)
{
...
uint32 HyGui::GetSpacerSize(HySpacerHandle hSpacer)
{
...
bool HyGui::SetSpacerSize(HySpacerHandle hSpacer, HySizePolicy eSizePolicy, uint32 uiSizeHint)
{
	if(m_SubSpacerMap.find(hSpacer) != m_SubSpacerMap.end())
	{
...
bool HyGui::RemoveSpacer(HySpacerHandle hSpacer)
{
	if(m_SubSpacerMap.find(hSpacer) != m_SubSpacerMap.end())
	{
...
HyLayoutHandle HyGui::InsertLayout(HyOrientation eNewLayoutType, HyLayoutHandle hInsertInto /*= HY_UNUSED_HANDLE*/)
{
...
glm::ivec2 HyGui::GetLayoutSize(HyLayoutHandle hLayout) const
{
...
bool HyGui::SetLayoutMargin(int16 iLeft, int16 iBottom, int16 iRight, int16 iTop, HyLayoutHandle hAffectedLayout /*= HY_UNUSED_HANDLE*/)
{
	...
	HyLogWarning("HyGui::SetLayoutMargins could not find specified layout: " << hAffectedLayout);
	return false;
}
...
bool HyGui::SetLayoutWidgetSpacing(int32 iWidgetSpacing, HyLayoutHandle hAffectedLayout /*= HY_UNUSED_HANDLE*/)
{
	...
	HyLogWarning("HyGui::SetLayoutWidgetSpacing could not find specified layout: " << hAffectedLayout);
	return false;
}

bool HyGui::RemoveLayout(HyLayoutHandle hLayout)
{
	if(m_SubLayoutMap.find(hLayout) == m_SubLayoutMap.end())
		return false;
...
int32 HyGui::GetDefaultWidgetSpacing() const
{
	return m_iDefaultWidgetSpacing;
}

void HyGui::SetDefaultWidgetSpacing(int32 iSpacing, bool bSetRootLayout)
{
...
void HyGui::EnableScrollBars(bool bUseVert, bool bUseHorz)
{
...
void HyGui::ScrollTo(float fVertScrollPos, float fHorzScrollPos)
{
...
std::vector<IHyWidget *> HyGui::AssembleWidgetList()
{
	...
	std::function<void(HyLayout *)> fpRecursive = [&](HyLayout *pLayout) {
		...
		{
			IHyGuiBase *pItem = static_cast<IHyGuiBase *>(pLayout->ChildGet(i));
...
bool HyGui::RequestWidgetFocus(IHyWidget *pWidget)
{
	IHyWidget *pFocusedWidget = GetFocusedWidget();
...
/*static*/ void HyGui::DistrubuteTextInput(std::string sText)
{
	for(uint32 i = 0; i < static_cast<uint32>(sm_pContainerList.size()); ++i)
	{
		IHyWidget *pFocusedWidget = sm_pContainerList[i]->GetFocusedWidget();
...
/*static*/ void HyGui::DistrubuteKeyboardInput(HyKeyboardBtn eBtn, HyBtnPressState eBtnState, HyKeyboardModifer iMods)
{
...
/*static*/ void HyGui::OnScroll(HyScrollBar *pSelf, float fNewPosition, float fTotalRange, void *pData)
{
	HyGui *pThis = static_cast<HyGui *>(pData);
