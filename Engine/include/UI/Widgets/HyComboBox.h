...
protected:
	...
	{
		COMBOBOXATTRIB_IsExpanded		= 1 << 18,
		COMBOBOXATTRIB_IsTransition		= 1 << 19,
		COMBOBOXATTRIB_STATEMASK = (COMBOBOXATTRIB_IsExpanded | COMBOBOXATTRIB_IsTransition),

		COMBOBOXATTRIB_IsHorzExpand		= 1 << 20,
		COMBOBOXATTRIB_IsPositiveExpand = 1 << 21,
		COMBOBOXATTRIB_IsInstantExpand	= 1 << 22,

		COMBOBOXATTRIB_IsExpandMouseDwn = 1 << 23,		// While expanded, keeps track of a left mouse click (anywhere) to retract the combo box upon release
		COMBOBOXATTRIB_NeedsRetracting	= 1 << 24,		// While expanded, this flag indicates that this combo box should retract on the next update. (This allows sub btn callbacks to occur before getting disabled on the retract)

		COMBOBOXATTRIB_FLAG_NEXT		= 1 << 25
	};
...
public:
	...
	uint32 InsertSubButton(const HyPanelInit &panelInit, const HyNodePath &textNodePath, HyButtonClickedCallback fpCallBack, void *pParam = nullptr, std::string sAudioPrefix = "", std::string sAudioName = "");
	uint32 InsertSubButton(const HyPanelInit &panelInit, const HyNodePath &textNodePath, int32 iTextMarginLeft, int32 iTextMarginBottom, int32 iTextMarginRight, int32 iTextMarginTop, HyButtonClickedCallback fpCallBack, void *pParam = nullptr, std::string sAudioPrefix = "", std::string sAudioName = "");
