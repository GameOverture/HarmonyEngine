...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// NOTICE: I REMOVED THE TIMESTAMP FUNCTIONALITY -JJK
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
static HyChar * getDeviceDescription(UINT joystickID, JOYCAPS caps) {
	HyChar * description = NULL;
	HyChar subkey[REG_STRING_MAX];
	HKEY topKey, key;
	LONG result;
	
	wsprintf(subkey, HyStr("%s\\%s\\%s"), REGSTR_PATH_JOYCONFIG, caps.szRegKey, REGSTR_KEY_JOYCURR);
	...
	if (result == ERROR_SUCCESS) {
		HyChar value[REG_STRING_MAX];
		HyChar name[REG_STRING_MAX];
		DWORD nameSize;
		
		wsprintf(value, HyStr("Joystick%d%s"), joystickID + 1, REGSTR_VAL_JOYOEMNAME);
		nameSize = sizeof(name);
		result = RegQueryValueEx(key, value, NULL, NULL, (LPBYTE) name, &nameSize);
		RegCloseKey(key);
		
		if (result == ERROR_SUCCESS) {
			wsprintf(subkey, HyStr("%s\\%s"), REGSTR_PATH_JOYOEM, name);
			...
			if (result == ERROR_SUCCESS) {
				...
				if (result == ERROR_SUCCESS) {
					description = (wchar_t *)malloc(nameSize);
	...
	description = (wchar_t *)malloc(lstrlen(caps.szPname) + 1);
	lstrcpy(description, caps.szPname);
...
void Gamepad_detectDevices() {
	...
	for (deviceIndex = 0; deviceIndex < numPadsSupported; deviceIndex++) {
		...
		if (joyGetPosEx(joystickID, &info) == JOYERR_NOERROR &&
			...
			deviceRecord->buttonStates = calloc(sizeof(int), deviceRecord->numButtons);
...
static void handleAxisChange(struct Gamepad_device * device, int axisIndex, DWORD ivalue) {
	...
	if (Gamepad_axisMoveCallback != NULL) {
		Gamepad_axisMoveCallback(device, axisIndex, value, lastValue, Gamepad_axisMoveContext);
	}
}
	...
	for (buttonIndex = 0; buttonIndex < device->numButtons; buttonIndex++) {
		if ((lastValue ^ value) & (1 << buttonIndex)) {
			...
			if (down && Gamepad_buttonDownCallback != NULL) {
				Gamepad_buttonDownCallback(device, buttonIndex, Gamepad_buttonDownContext);
			} else if (!down && Gamepad_buttonUpCallback != NULL) {
				Gamepad_buttonUpCallback(device, buttonIndex, Gamepad_buttonUpContext);
			}
		}
	}
}
...
static void handlePOVChange(struct Gamepad_device * device, DWORD lastValue, DWORD value) {
	...
	if (newX != lastX) {
		device->axisStates[devicePrivate->povXAxisIndex] = (float)newX;
		if (Gamepad_axisMoveCallback != NULL) {
			Gamepad_axisMoveCallback(device, devicePrivate->povXAxisIndex, (float)newX, (float)lastX, Gamepad_axisMoveContext);
		}
	}
	if (newY != lastY) {
		device->axisStates[devicePrivate->povYAxisIndex] = (float)newY;
		if (Gamepad_axisMoveCallback != NULL) {
			Gamepad_axisMoveCallback(device, devicePrivate->povYAxisIndex, (float)newY, (float)lastY, Gamepad_axisMoveContext);
		}
	}
}
