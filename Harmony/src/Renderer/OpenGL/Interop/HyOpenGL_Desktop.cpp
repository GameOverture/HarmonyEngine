...
HWND HyOpenGL_Desktop::ConstructWindow(const HyWindowInfo &wndInfo)
{
...
HWND HyOpenGL_Desktop::GetHWND(int32 iWindowIndex)
{
	for(uint32 i = 0; i < m_RenderSurfaceList.size(); ++i)
	{
...
/*virtual*/ void HyOpenGL_Desktop::StartRender()
{
	if(m_RenderSurfaceIter->GetType() == HYRENDERSURFACE_Window)
	{
		...
		HyErrorCheck_OpenGL("HyOpenGL_Desktop::StartRender", "wglMakeCurrent");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		case WM_CREATE: {
			...
			HyOpenGL_Desktop *pThis = reinterpret_cast<HyOpenGL_Desktop *>(pCreateStruct->lpCreateParams);
		...
		case WM_DESTROY: {
			HyOpenGL_Desktop *pThis = reinterpret_cast<HyOpenGL_Desktop *>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
