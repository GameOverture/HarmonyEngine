...
namespace HyBitManip
{
	...
	inline uint16 GetHighInt(uint32 uiValue)
	{
		return (uiValue & 0xFFFF0000) >> 16;
	}
	inline uint16 GetLowInt(uint32 uiValue)
	{
		return (uiValue & 0x0000FFFF);
	}

	inline uint8 GetHighInt(uint16 uiValue)
	{
		return (uiValue & 0xFF00) >> 8;
	}
	inline uint8 GetLowInt(uint16 uiValue)
	{
		return (uiValue & 0x00FF);
	}
