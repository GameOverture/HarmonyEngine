...
{
	void *pageStart = (void*)((uint32)ptr & ~(kSmlPageSize-1));  // assume all small pages are aligned to their own size addresses, or better
...
{
	void *pageStart = (void*)((uint32)ptr & ~(kMedPageSize-1));  // assume all medium pages are aligned to their own size addresses, or better
